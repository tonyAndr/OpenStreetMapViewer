...
public class GeoMethods implements AppConstants {
        ...
        if (location != null) {
            ...
            if (stage_id == 0) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    LatLngBounds box = getStageBBox(fileObj);
    ...
    private LatLngBounds getStageBBox(JSONObject obj) throws JSONException {
        JSONObject min,max;
        max = obj.getJSONObject("maxlatlng");
        min = obj.getJSONObject("minlatlng");
        return new LatLngBounds((new LatLng(min.getDouble("lat"), min.getDouble("lng"))),(new LatLng(max.getDouble("lat"), max.getDouble("lng"))));
    }

    public BoundingBox getStageMFBBox(int stageid) throws JSONException {
        JSONObject min,max;
        JSONArray jsonArray = jfh.parseJSONArr("json/bounds.json");
        JSONObject fileObj = jsonArray.getJSONObject(stageid-1);

        max = fileObj.getJSONObject("maxlatlng");
        min = fileObj.getJSONObject("minlatlng");
        return new BoundingBox(min.getDouble("lat"), min.getDouble("lng"),max.getDouble("lat"), max.getDouble("lng"));
    }
    public LatLngBounds getStageGoogleBBox(int stageid) throws JSONException {
        JSONObject min,max;
        JSONArray jsonArray = jfh.parseJSONArr("json/bounds.json");
        JSONObject fileObj = jsonArray.getJSONObject(stageid-1);
