...
//        mCurrentLocation = new Location("");
        ...
        Log.w(DEBUGTAG, "contains: " + mPrefs.contains(MAPTYPE_PREF));
        Log.w(DEBUGTAG, "type: " + mPrefs.getInt(MAPTYPE_PREF, MAPTYPE_PREF_NORMAL));
        if (mPrefs.getInt(MAPTYPE_PREF, MAPTYPE_PREF_NORMAL) == MAPTYPE_PREF_HYBRID) {
            ...
            if (mCurrentLocation != null) {
                if (areaLimitSpainGMap.contains(new LatLng(mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude()))) {
                    ...
                    if (bundle.getBoolean("globe", false) && bundle.getBoolean("near", false)) {
                        moveCameraOnLoad(mCurrentLocation, SHOW_STAGE_ZOOM_LEVEL);
//                        map.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude()), SHOW_STAGE_ZOOM_LEVEL));
                        ...
                        moveCameraOnLoad(mFinishLocation, TRACK_ZOOM_LEVEL);
//                        map.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(mFinishLocation.getLatitude(), mFinishLocation.getLongitude()), TRACK_ZOOM_LEVEL));
                    ...
                    if (bundle.getBoolean("globe", false)) {
                        moveCameraOnLoad(geoMethods.getStageGoogleBBox(bundle.getInt("stage_id")));
...
//                map.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude()), SHOW_STAGE_ZOOM_LEVEL));
    ...
    private void moveCameraOnLoad(final Location location, final int zoom) {
        map.setOnMapLoadedCallback(new GoogleMap.OnMapLoadedCallback() {
            @Override
            public void onMapLoaded() {
    ...
    private void moveCameraOnLoad(final LatLngBounds latLngBounds) {
        map.setOnMapLoadedCallback(new GoogleMap.OnMapLoadedCallback() {
            @Override
            public void onMapLoaded() {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        inflater.inflate(R.menu.menu_gmap, menu);
        for (int i=0; i < menu.size(); i++) {
            MenuItem mi = menu.getItem(i);
            if (mi.getItemId() == R.id.gmap_type) {
                if (mPrefs.getInt(MAPTYPE_PREF, MAPTYPE_PREF_NORMAL) == MAPTYPE_PREF_HYBRID) {
                    mi.setChecked(true);
                } else {
                    mi.setChecked(false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.gmap_type) {
//            new MapTypeDialogFragment().show(getActivity().getFragmentManager(),GMS_FRAGMENT_MAPTYPE_TAG);

            if (item.isChecked()) {
                item.setChecked(false);
                changeType(MAPTYPE_PREF_NORMAL);
            } else {
                item.setChecked(true);
                changeType(MAPTYPE_PREF_HYBRID);
            }
            return true;
        }
    ...
    private void changeType(int type) {
        SharedPreferences.Editor editor = mPrefs.edit();
        map.setMapType(type == MAPTYPE_PREF_HYBRID ? GoogleMap.MAP_TYPE_HYBRID : GoogleMap.MAP_TYPE_NORMAL);
        editor.putInt(MAPTYPE_PREF,type == MAPTYPE_PREF_HYBRID ? MAPTYPE_PREF_HYBRID : MAPTYPE_PREF_NORMAL);

        Log.w(DEBUGTAG, "value " + type);
        Log.w(DEBUGTAG, "commit " + editor.commit());
    }
